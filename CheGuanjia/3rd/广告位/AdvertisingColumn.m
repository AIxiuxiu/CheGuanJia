...
- (void)setArray:(NSArray *)imgArray{
    ...
    if (_totalNum>0)
    {
        for (int i = 0; i<_totalNum; i++) {
            _img = [[UIImageView alloc]initWithFrame:CGRectMake(i*CGRectGetWidth(_scrollView.frame), 0, CGRectGetWidth(_scrollView.frame), CGRectGetHeight(_scrollView.frame))];
            _img.contentMode = UIViewContentModeScaleToFill;
            
            if ([imgArray[i] isKindOfClass:[NSString class] ]) {
                [_img setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",imgArray[i]]] placeholderImage:[UIImage imageNamed:@"photoNo"]];
            }else{
            _img.image = [UIImage imageNamed:imgArray[i]];
            }
            [_img setTag:i];
        ...
        _img = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth(_scrollView.frame), CGRectGetHeight(_scrollView.frame))];
        [_img setImage:[UIImage imageNamed:@"photoNO"]];
